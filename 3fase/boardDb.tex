...
\textbf{Triggers:} O ficheiro com os triggers chama-se triggers.sql. E lá podemos encontrar três pares de gatilhos que impossibilitam a inserção ou atualização de informação inválida.
\begin{itemize}
  \item Um arbitro só pode arbitrar uma partida se tiver competências para tal (Update e Insert)
  \item Um jogador so pode estar numa equipa num torneio (Update e Insert)
  \item
\end{itemize}
